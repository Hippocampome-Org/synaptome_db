...
(SELECT cond_max.g as max_g
	FROM
		max_cond2 AS cond_max
	WHERE
		tm_cond3.pre = cond_max.`Presynaptic Neuron` AND
		tm_cond3.post = cond_max.`Postsynaptic Neuron`
),
`max_tau_d` =
...
(SELECT cond_min.g as min_g
	FROM
		min_cond2 AS cond_min
	WHERE
		tm_cond3.pre = cond_min.`Presynaptic Neuron` AND
		tm_cond3.post = cond_min.`Postsynaptic Neuron`
),
`min_tau_d` =
...
(SELECT cond_means.g as means_g
	FROM
		means_cond2 AS cond_means
	WHERE
		tm_cond3.pre = cond_means.`Presynaptic Neuron` AND
		tm_cond3.post = cond_means.`Postsynaptic Neuron`
),
`means_tau_d` =
...
(SELECT cond_std.g as std_g
	FROM
		std_cond2 AS cond_std
	WHERE
		tm_cond3.pre = cond_std.`Presynaptic Neuron` AND
		tm_cond3.post = cond_std.`Postsynaptic Neuron`
),
`std_tau_d` =
